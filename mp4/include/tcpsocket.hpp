#ifndef MP4_TCP_SOCKET
#define MP4_TCP_SOCKET
...
public:
  ...
  /**
   *   Send a custom string through TCP socket
   *   Return 1 means there is an exception and socket is broken.
   */ 
  int sendStr(string in);

  int recvStr(string &in);

  int sendFile(string fname);

  int recvFile(string fname);
