...
def start(id):
    ...
    os.system("ssh %s \'pkill -u %s server; rm -f *.log; nohup ./mp4/server > /dev/null 2>&1 & \' " % (server, USER))
    #os.system("ssh %s \'pkill -u %s server; rm -f server*.log; nohup ./mp4/server > /dev/null 2>&1 & \' " % (server, USER))
