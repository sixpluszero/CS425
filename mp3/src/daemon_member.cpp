...
void Daemon::setMasterList(string w) {
    master_list.clear();
    int pIdx = w.find(",");
    w = w.substr(pIdx+1, w.length());
    while (w.length()) {
        pIdx = w.find(",");
        if (pIdx == -1) pIdx = w.length();
        string sub = w.substr(0, pIdx);
        int sIdx = sub.find("/");
        master_list[stoi(sub.substr(0, sIdx))] = sub.substr(sIdx+1, sub.length());
        if (pIdx < int(w.length())) {
            w = w.substr(pIdx+1, w.length());
        } else {
            break;
    ...
    plog("init master: %s", mastersToString().c_str());
}
